            ...
            {enrolledCourses.map((course, index) => (
              <tr key={index} className="border-b border-gray-500/20">
                ...
                <td className="px-4 py-3 max-sm:text-right ">
                  <button
                    className={`px-4 sm:px-6 py-2 sm:py-2.5 rounded-2xl font-semibold text-white text-sm sm:text-base  transition-all duration-300 ease-in-out cursor-pointer
                    ${
                      progressArray[index] &&
                      progressArray[index].lectureCompleted /
                        progressArray[index].totalLecture ===
                        1
                        ? "bg-blue-800 hover:bg-blue-900" // Completed
                        : "bg-blue-400 hover:bg-blue-500" // On Going
                    }`}
                  on onClick={()=> navigate('/player/'+ course._id)}>
                    {progressArray[index] &&
                    progressArray[index].lectureCompleted /
                      progressArray[index].totalLecture ===
                      1
